...
import { ZONE1_MONTHS } from "./timelineBounds";
...
export default function ArtistDot({
  ...
  const monthLabel = ZONE1_MONTHS[artist.releaseMonth];
  ...
  const effectiveY = showAlbumArt ? 16 : activeY;
  const albumSize = 152;
  const nameY = effectiveY + artistSize + 6;
  const albumY = nameY + 22;

  // Determine if tooltip should show below (if artist is near the top)
  const showTooltipBelow = effectiveY < 92;
