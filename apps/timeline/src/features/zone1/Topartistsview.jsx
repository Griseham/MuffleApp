...
import { EXTENDED_TIMELINE_MONTHS, CURRENT_MONTH_INDEX, TIMELINE_MONTHS } from "../../backend/timelineMockData";
...
export default function TopArtistsView({
  ...
  const [visibleYear, setVisibleYear] = useState(TIMELINE_MONTHS[CURRENT_MONTH_INDEX]?.year ?? 2025);
  ...
  const timelineWidth = EXTENDED_TIMELINE_MONTHS.length * MONTH_WIDTH;
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const month = EXTENDED_TIMELINE_MONTHS[Math.floor(centerX / MONTH_WIDTH)];
