...
const UserHoverCard = ({ user, userData, className = "" }) => {
  ...
  const HorizontalBar = ({ genre }) => (
    <div className="mb-3">
      <div className="flex justify-between mb-1 text-[13px]">
        ...
        <span className="text-gray-200 font-semibold tabular-nums">{genre.percent}%</span>
      </div>
  ...
  const ArrowWithCount = ({ direction, strength, count }) => {
    const color = direction === 'positive' ? 'text-green-400' : 'text-red-400';
    
    return (
      <div className="flex items-center">
        <div className={`flex-shrink-0 w-8 h-8 rounded bg-gray-900/90 flex items-center justify-center border border-gray-700 ${color}`}>
          {direction === 'positive' && (
        ...
        <div className={`ml-1.5 text-sm leading-none font-bold tabular-nums ${color}`}>
          {count}
        </div>
      </div>
    );
    ...
    <div className={`bg-black border border-gray-800 rounded-xl shadow-xl overflow-hidden backdrop-blur-sm ${className}`}>
      <div className="p-4">
        ...
        <div className="flex items-center mb-3">
          ...
          <div className="ml-auto flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/12 border border-emerald-400/35 text-emerald-300">
            <VolumeIcon className="w-5 h-5" />
            <span className="text-base font-bold tabular-nums leading-none">{volume}</span>
          </div>
        </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="text-white text-sm font-medium">
                Discovered <span className="font-bold text-base tabular-nums">{discoveryPercent}%</span> of music
              </div>
              <svg 
        ...
        <div className="bg-gray-900/40 p-3 rounded-lg mb-3">
          ...
          <div className="mb-3 pb-2 border-b border-gray-800">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-green-300 text-sm font-semibold tabular-nums ml-2">{positivePercent}%</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-red-300 text-sm font-semibold tabular-nums ml-2">{negativePercent}%</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between text-sm border-t border-gray-800 pt-4 pb-1">
          <div className="mr-4">
            <span className="font-extrabold text-white text-lg tabular-nums">{following}</span>
            <span className="text-gray-300 text-xs uppercase tracking-wide ml-1">Following</span>
          </div>
          <div className="mr-4">
            <span className="font-extrabold text-white text-lg tabular-nums">{followers}</span>
            <span className="text-gray-300 text-xs uppercase tracking-wide ml-1">Followers</span>
          </div>
