...
const MOBILE_QUEUE_MEDIA_QUERY = "(max-width: 480px)";
const getInitialIsMobile = () => {
  if (typeof window === "undefined" || typeof window.matchMedia !== "function") {
    return false;
  }
  return window.matchMedia(MOBILE_QUEUE_MEDIA_QUERY).matches;
};
...
// Cleaner speaker icon for QL volume value (more legible at small sizes)
const QueueVolumeIcon = ({ className = "" }) => (
  <svg
    className={className}
    viewBox="0 0 20 20"
    fill="currentColor"
    aria-hidden="true"
  >
    <path d="M11.5 3.5a.75.75 0 00-1.28-.53l-3.9 3.9H3.75A1.75 1.75 0 002 8.62v2.76c0 .97.78 1.75 1.75 1.75h2.57l3.9 3.9a.75.75 0 001.28-.53v-13z" />
    <path d="M14.03 6.97a.75.75 0 011.06 0 4.25 4.25 0 010 6.06.75.75 0 11-1.06-1.06 2.75 2.75 0 000-3.94.75.75 0 010-1.06z" />
  </svg>
);
...
const QueueLine = ({ qlUsers }) => {
  ...
  // Track hover card lifecycle for smoother open/close transitions
  const [hoverCardUserId, setHoverCardUserId] = useState(null);
  const [isHoverCardVisible, setIsHoverCardVisible] = useState(false);
  const hoverCloseDelayRef = useRef(null);
  const hoverUnmountRef = useRef(null);

  // Detect mobile view to disable hover cards
  ...
  useEffect(() => {
    ...
    const mediaQuery = window.matchMedia(MOBILE_QUEUE_MEDIA_QUERY);
  ...
  const clearHoverTimers = useCallback(() => {
    if (hoverCloseDelayRef.current) {
      window.clearTimeout(hoverCloseDelayRef.current);
      hoverCloseDelayRef.current = null;
    }
    if (hoverUnmountRef.current) {
      window.clearTimeout(hoverUnmountRef.current);
      hoverUnmountRef.current = null;
    }
  }, []);

  const openHoverCard = useCallback((userId) => {
    clearHoverTimers();
    setHoverCardUserId(userId);
    setIsHoverCardVisible(true);
  }, [clearHoverTimers]);

  const scheduleCloseHoverCard = useCallback(() => {
    clearHoverTimers();
    hoverCloseDelayRef.current = window.setTimeout(() => {
      setIsHoverCardVisible(false);
      hoverUnmountRef.current = window.setTimeout(() => {
        setHoverCardUserId(null);
        hoverUnmountRef.current = null;
      }, HOVER_FADE_MS);
      hoverCloseDelayRef.current = null;
    }, HOVER_CLOSE_DELAY_MS);
  }, [HOVER_CLOSE_DELAY_MS, HOVER_FADE_MS, clearHoverTimers]);

  useEffect(() => () => clearHoverTimers(), [clearHoverTimers]);

  const hoveredUser = staticUserData.find((user) => user.id === hoverCardUserId);
          ...
          return (
            <div key={user.id} className="ql-row-wrapper">
                ...
                <div
                  ...
                  onMouseEnter={!isMobile ? () => openHoverCard(user.id) : undefined}
                  onMouseLeave={!isMobile ? scheduleCloseHoverCard : undefined}
                  onClick={(e) => e.stopPropagation()}
                >
                  <div
                    className={`ql-row__avatar ${
                      !isMobile && hoverCardUserId === user.id ? 'is-selected' : ''
                    } ${
                      index === 0 ? 'is-gold' :
                      index === 1 ? 'is-silver' :
                      index === 2 ? 'is-bronze' : ''
                  ...
                  {!isMobile && hoverCardUserId === user.id && hoveredUser && (
                    <div
                      className={`ql-row__hovercard ${isHoverCardVisible ? "is-open" : ""}`}
                      onMouseEnter={() => openHoverCard(user.id)}
                ...
                <span className="ql-row__volume">
                  <QueueVolumeIcon className="ql-row__volume-icon" />
                  {user.volume}
                </span>
