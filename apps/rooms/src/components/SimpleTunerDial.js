...
const SimpleTunerDial = ({
  ...
  const hasMobileInstructionSteps = Array.isArray(mobileInstructionSteps) && mobileInstructionSteps.length > 0;
  const hintText = desktopHintText || (
    dialLocked
  ...
  return (
    <div
      className={[
        'simple-tuner-dial',
        disabled ? 'disabled' : '',
        dialLocked ? 'dial-locked' : '',
        hasMobileInstructionSteps ? 'simple-tuner-dial--has-mobile-steps' : '',
      ].join(' ').trim()}
    >
      <div className="simple-tuner-dial__content">
        ...
        <div className="simple-tuner-dial__dial-wrapper">
          <div className="simple-tuner-dial__dial-and-steps">
            <div
              ref={dialRef}
            ...
            >
              ...
              <div className="simple-tuner-dial__hint">{hintText}</div>
            </div>

            {hasMobileInstructionSteps && (
              <div className="simple-tuner-dial__mobile-steps" aria-label="Tune instructions">
                {mobileInstructionSteps.map((step, index) => (
                  <div key={`step-${index}`} className="simple-tuner-dial__mobile-step">
                    {step}
                  </div>
                ))}
              </div>
