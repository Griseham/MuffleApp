...
export const useEnhancedRoomFeed = (selectedArtists = [], options = {}) => {
  const configuredInitialSimilarity = Number.isFinite(options.initialSimilarity)
    ? Math.max(-1000, Math.min(1000, options.initialSimilarity))
    : 80;
  // State
  const [rooms, setRooms] = useState([]);
  const [relatedArtists, setRelatedArtists] = useState([]);
  const [randomArtists, setRandomArtists] = useState([]);
  const [currentSimilarity, setCurrentSimilarity] = useState(configuredInitialSimilarity);
  ...
  const lastGenerationKeyRef = useRef(null);
  ...
  const fetchRandomArtists = useCallback(async () => {
    ...
    try {
      const random = await fetchRandomGenreArtists(50);
  ...
  const generateRooms = useCallback(async (primaryValue, targetSecondaryValue = null, mode = 'similarity') => {
    const generationKey = `${mode}:${primaryValue}:${targetSecondaryValue ?? 'none'}`;
  ...
  useEffect(() => {
    if (!loadingState.initialLoad && !loadingState.similarArtists) {
      const nextKey = `similarity:${debouncedSimilarity}:${targetVolumeRef.current ?? 'none'}`;
      if (lastGenerationKeyRef.current === nextKey) return;
