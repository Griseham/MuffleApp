...
import { TOTAL_WIDTH, TOTAL_HEIGHT } from './starfieldCore';
...
const searchForArtists = useCallback(async (searchQuery) => {
  ...
  try {
    ...
    // Step 2: Search for artists using the token
  ...
  const handleSearchChange = useCallback(async (e) => {
    ...
    if (value.trim().length >= 2) {
      const results = await searchForArtists(value);
...
const handleAddArtist = useCallback((artist) => {
  ...
  if (activeArtists.some(a => a.id === artist.id || a.name.toLowerCase() === artist.name.toLowerCase())) {
    setError(`Artist "${artist.name}" is already added`);
    setTimeout(() => setError(''), 2000);
    return;
  }
  ...
  // Extend the artist object with additional useful info
