/**
 * Avatar Service - Manages random avatar assignment from public/assets/image1.png to image1000.png
...
/**
 * Get a random avatar URL from the 1000 available images
...
export const getAvatarForUser = (userId) => {
  ...
  const imageNumber = Math.abs(hash % 1000) + 1;
...
export const getRandomAvatars = (count) => {
  ...
  for (let i = 0; i < count; i++) {
    let imageNumber;
    do {
      imageNumber = Math.floor(Math.random() * 1000) + 1;
