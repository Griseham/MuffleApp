// RadioTuner.js - Enhanced with landed-on frequency detection
  ...
  for (let b = 0; b < bands; b++) {
    ...
    for (let i = 0; i < count * 2; i++) { // Try more attempts to get good distribution
      ...
      if (absPos <= MAX_VOLUME) {
        ...
        // Ensure a good variety of frequency values
        if (absPos >= 2400) {
          freq = Math.floor(hash * 60) + 40; // 40-99 range
        } else if (absPos >= 1600) {
          freq = Math.floor(hash * 50) + 30; // 30-79 range
        } else if (absPos >= 800) {
          freq = Math.floor(hash * 40) + 20; // 20-59 range
        } else {
          freq = Math.floor(hash * 30) + 10; // 10-39 range
        }
...
useEffect(() => {
  ...
  throttledOnChange({
    ...
    bandIndex: idx,
    bandFreqs: points,
    landedFreq: landedPoint?.freq || null,
...
const frequencyPointsToRender = useMemo(() => {
  ...
  // Render
  return (
    <div className={`rt-wrapper ${className}`} style={{ '--rt-width': `${rulerWidth}px` }}>
